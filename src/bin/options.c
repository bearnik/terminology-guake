...
// implementation for bind functions
static void
_cb_op_bind(void *data __UNUSED__, Evas_Object *obj __UNUSED__, void *event __UNUSED__)
{
   if (mode == 8) return;
   mode = 8;
   edje_object_signal_emit(saved_bg, "optdetails,hide", "terminology");
   ...
   switch (mode)
     {
      ...
      case 8: options_bind(op_opbox, data); break;
      default: break;
     }
   ...
   if (!op_frame)
     {
        ...
        elm_toolbar_item_append(o, "preferences-color",              "Binds",     _cb_op_bind,      term);
