...
void
main_bind_toggle_update(const Config *config)
{
   if (config->bind)
     {
        edje_object_signal_emit(bg, "bind,on", "terminology");
        //elm_win_alpha_set(win, EINA_TRUE);
        //evas_object_hide(backbg);
     }
   else
     {
        edje_object_signal_emit(bg, "bind,off", "terminology");
        //elm_win_alpha_set(win, EINA_FALSE);
        //evas_object_show(backbg);
     }
}
...
void
main_bind_expand_update(const Config *config)
{
   if (config->expand)
     {
        edje_object_signal_emit(bg, "expand,on", "terminology");
       // elm_win_alpha_set(win, EINA_TRUE);
       // evas_object_hide(backbg);
     }
   else
     {
        edje_object_signal_emit(bg, "expand,off", "terminology");
       // elm_win_alpha_set(win, EINA_FALSE);
       // evas_object_show(backbg);
     }
}
...
elm_main(int argc, char **argv)
{
   ...
   Ecore_Getopt_Value values[] = {
    ...
    /* Bind and Expand values */ 
     ECORE_GETOPT_VALUE_BOOL(bind),
     ECORE_GETOPT_VALUE_BOOL(expand),
   ...
   if(config->bind)
   {
      int max_width, max_height,max_xx, max_yy;
      elm_win_screen_size_get(win, &max_xx, &max_yy, &max_width,&max_height);

      evas_object_move(win, 1, -10);      /* Moving to the top x,y coordinates */
      // testing the width
      size_w = 180;  /* Have to see how to get the max width */ 
